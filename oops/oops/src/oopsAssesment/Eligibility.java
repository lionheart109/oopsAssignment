...
public class Eligibility {
    ...
    public static boolean isUgCgpaEligible(Candidate candidate) {
        return candidate.getUndergraduateScore() >= 7.0;
    }

    public static boolean isPgCgpaEligible(Candidate candidate) {
        if (candidate.getUndergraduateScore() != 0.0) {
            return candidate.getPostgraduateScore() >= 7.0;
        }
        return true;
    }

    public static boolean isFullTimeEligible(Candidate candidate) {
        if (candidate.getUndergraduateScore() >= 8.0 && candidate.getProjectsDone() >= 2) {
            return true;
        }
        if (candidate.getUndergraduateScore() >= 8.0 && candidate.getInterviewScore() >= 35) {
            return true;
        }
        return false;
    }
